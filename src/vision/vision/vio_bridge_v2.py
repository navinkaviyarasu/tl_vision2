...
class RosOdometryPublisher(rclpy.node.Node):
    def __init__(self, vision_module_type, yaw, pitch, roll):
...
def main():
    ...
    # parameter to normalize the mounting orientation of the vision module
    yaw, pitch, roll = map(float, input("""Enter the mounting orientation of the vision module with respect 
                                        to the body frame FRD in order of yaw, pitch and roll (in degrees): """).split())

    print(f"Yaw:{yaw}\nPitch:{pitch}\nRoll:{roll}\n\n")

    # parameter to select the type/make of the vision module
    # as the coordinate from orientation of the vk180 & vk180pro are different
    # NOTE: where the modules will be mounted should be also taken care like forward facing or backward facing
    vision_module_type = None
    ...
    rclpy.init()
    ros_odometry_publisher = RosOdometryPublisher(vision_module_type,yaw,pitch,roll)
